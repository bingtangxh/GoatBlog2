...
2. 路径任意，但是虚拟硬盘格式一定要是 VHD，虚拟硬盘类型一定要是“固定大小”，虚拟硬盘大小取决于你 Azure 虚拟机的硬盘大小，最好完全一样。如果你是学生订阅的免费试用虚拟机，那一般是 64GB。
3. 新建硬盘之后，将硬盘初始化为 GPT 分区表并新建一个简单卷。简单卷大小建议 16GB 以上，一般都是 64GB 用满。
4. 用你自己喜欢或者能用的方法将 Windows Server 2012 R2 的 install.wim/esd 文件提取出来，可以是在 Windows 8 及以上双击挂载 ISO 或者用解压缩软件提取出来。
5. 用 Dism++ 将 wim/esd 文件释放到虚拟硬盘里，注意选择好 wim/esd 文件后、开始释放之前，切记选择 Windows 版本为 Windows Server 2012 R2 STANDARD 桌面体验（DESKTOP EXPERIENCE），切勿选择 DATACENTER 或者没有“桌面体验”的版本，并且一定要勾选“添加引导”。
6. 上一个步骤也可以用 dism 和 BCDBOOT 指令，具体可自行网络搜索（哈，你不一定搜得到，因为网上可能一搜全是让你用 Dism++）
7. 等待十几分钟进行释放，完成之后，再打开磁盘管理，将虚拟硬盘“分离”。
...
## 将 vhd 文件上传上去
（WIP……）
大概就是可以 Web 端直接上传（稳定性不好且无法断点续传，可能浏览器报错内存不足），或者给 Blob 新建一个能进行写入操作的 token ，再下载一个 Azure 还是什么的的本地管理客户端，用 token 连接上，再用这个客户端上传上去。
